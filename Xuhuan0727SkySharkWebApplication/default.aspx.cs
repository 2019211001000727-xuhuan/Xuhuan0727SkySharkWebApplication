...
namespace Xuhuan0727SkySharkWebApplication
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                conn.Open();
                String queryString = "Select UserNamme, Password,role from dtUsers where UserName='" + username + "'";
                ...
                else
                {
                    if(ds1.Tables["dtUsers"].Rows[0][1].ToString().Trim() == txtPassword.Text.Trim())
                    {
                        ...
                        if(Role == "Disabled")
                        {
                            lblMessage.Text = "Yours account has been disabled.Please contact the network Administrator";
                            return;
                        }
                        switch(Role)
                        {
